...
import { UpdateMatchScoreDialog } from './MatchScoreDialog';
  ...
  return (
    <>
      <div className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50 transition-colors">
        {/* Player 1 */}
        <div className={`flex-1 flex items-center justify-end gap-4 ${isPlayer1Winner ? 'font-bold' : ''}`}>
          <PlayerAvatar
            playerId={match.player1_id}
            size="md"
            isWinner={isPlayer1Winner}
          />
          <span className="text-lg">Player {match.player1_id}</span>
        </div>
        ...
        {/* Player 2 */}
        <div className={`flex-1 flex items-center justify-start gap-4 ${isPlayer2Winner ? 'font-bold' : ''}`}>
          <span className="text-lg">Player {match.player2_id}</span>
          <PlayerAvatar
